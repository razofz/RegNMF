Visualization.default <- function(wholef,
                                  peakf,
                                  regf,
                                  chr,
                                  from,
  ...
  x <- match(data$V4, chr)
  data <- data[!is.na(x), ]
  gr <- GenomicRanges::GRanges(seqnames = data$V4, ranges = IRanges(
    start = data$V5, end = data$V6
  ))
  ...
  Gene <- BiomartGeneRegionTrack(
    biomart = mart,
    # biomart = ensembl,
    chromosome = chr, start = 0, end = NULL, name = "Gene",
    # chromosome = chr, start = from, end = to, name = "Gene",
    # genome = "hg38", chromosome = chr, start = from,
    # end = to, name = "Gene",
  ...
  for (i in clusterlist) {
    ...
    x <- match(data$V1, chr)
    data <- data[!is.na(x), ]
    cluster[[i]] <- GRanges(seqnames = data$V1, ranges = IRanges(
      start = data$V2, end = data$V3
    ))
    atrack[[i]] <- AnnotationTrack(cluster[[i]])
  }

  ftmp <- factor(Gene@range@elementMetadata@listData$symbol)
  # ftmp <- factor(str_to_title(Gene@range@elementMetadata@listData$symbol))
  flevel <- levels(ftmp)
  ...
  for (i in clusterlist) {
    ...
    data <- data[!is.na(x), ]
    if (nrow(data) <= 0) {
      print(paste0("> No data for cluster ", i))
      next
    }
    # print(i)
    x <- as.integer(na.omit(x))
    data$V5 <- RL[x]
    REcluster[[i]] <- GRanges(seqnames = data$V1, ranges = IRanges(
      start = data$V2, end = data$V3
    ))
    RETG[[i]] <- GRanges(seqnames = rep(chr, length(data$V1)), IRanges(
      start = data$V5,
      end = data$V5 + 100
    ))
  ...
  # } else {
    ...
    for (i in clusterlist) {
      if (class(interaction_track[[i]]) == "NULL") {
        next
      }
